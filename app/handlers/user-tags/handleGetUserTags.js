...
////// THIS HANDLER //////
//////////////////////////
async function handleGetUserTags(app, req, res) {
  ...
  try {
    ...
    if (!(privileges.reception.view_tags === 1 || privileges.settings.super_admin === 1)) {
    ...
    const tagNamesClean = await userTags.map(function (tag) {
      ...
      let thisCreated = 0;
      if (tag.data.created) {
        thisCreated = tag.data.created;
      }

      let thisDescription = "";
      if (tag.data.description) {
        thisDescription = tag.data.description;
      }
